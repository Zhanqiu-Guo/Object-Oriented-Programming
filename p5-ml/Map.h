...
public:
  ...
  Value_type& operator[](const Key_type& k) {
    if (find(k) == pair_tree.end()) insert({k, Value_type()});
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    std::pair<Iterator, bool> insert_return;
    if (pair_tree.find(val) == end()) {
      pair_tree.insert(val);
      insert_return.second = true;
    } else {
      insert_return.second = false;
    }
    insert_return.first = pair_tree.find(val);
    return insert_return;
  }

  // EFFECTS : Returns an iterator to the first key-value pair in this Map.
  Iterator begin() const {
    return pair_tree.begin();
  }

  // EFFECTS : Returns an iterator to "past-the-end".
  Iterator end() const {
    return pair_tree.end();
  }

private:
  // Add a BinarySearchTree private member HERE.
  BinarySearchTree<Pair_type, PairComp> pair_tree;
};
