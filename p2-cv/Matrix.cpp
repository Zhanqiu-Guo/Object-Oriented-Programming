...
int Matrix_row(const Matrix* mat, const int* ptr) {
    int row;
    row = (ptr - mat->data)/Matrix_width(mat);
    return row;
}
...
int Matrix_column(const Matrix* mat, const int* ptr) {
    int row, col;
    row = Matrix_row(mat, ptr);
    col = (ptr - mat->data) -  row * mat->width;
    return col;
}
...
int* Matrix_at(Matrix* mat, int row, int column) {
    ...
    int* ptr;
    ptr = mat->data + row * Matrix_width(mat) + column;
    return ptr;
}
...
void Matrix_fill_border(Matrix* mat, int value) {
    ...
    for(int j = 0; j < Matrix_height(mat) - 1; ++j)
    {
...
int Matrix_max(const Matrix* mat) {
    int sofar = *Matrix_at(mat, 0, 0);
    for(int i = 0; i < Matrix_height(mat); ++i)
    {
        for (int j = 0; j < Matrix_width(mat); ++j)
        {
            if (*Matrix_at(mat, i, j) > sofar)
                sofar = *Matrix_at(mat, i, j);
        }
    }
    return sofar;
}
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row,
    ...
    int sofar = *Matrix_at(mat, row, column_start);
    int theCol = column_start;
    for(int i = column_start; i < column_end; ++i)
    {
        if (*Matrix_at(mat, row, i) < sofar)
        {
            sofar = *Matrix_at(mat, row, i);
            theCol = i;
        }
    }
    return theCol;
